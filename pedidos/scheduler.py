...
UMBRAL_INACTIVIDAD = datetime.timedelta(minutes=settings.INTERVALO_SYNC_PEDIDOS * 3)  # 3 corridas sin señal de vida = dueño anterior murió
...
def _tomar_lock():
    try:
        ...
        pass  # ya existe, ver si está viejo

    with transaction.atomic():
        lock = LockTarea.objects.select_for_update().filter(nombre=NOMBRE_LOCK).first()
        if lock and timezone.now() - lock.tomado_en > UMBRAL_INACTIVIDAD:
            logger.warning("Cron SAP: lock de PID %s inactivo hace rato, lo tomo yo (PID %s).", lock.pid, os.getpid())
            lock.pid = os.getpid()
...
def iniciar_scheduler():
    ...
    if settings.CRON_SAP_ACTIVO:
        scheduler.add_job(_job_sincronizar_sap, "interval", minutes=settings.INTERVALO_SYNC_PEDIDOS, id="sincronizar_sap")
    if settings.CRON_WOO_ACTIVO:
        scheduler.add_job(_job_sincronizar_woo, "interval", minutes=settings.INTERVALO_SYNC_PEDIDOS, id="sincronizar_woo")
    scheduler.start()
