...
#Variante de _template_despacho para un envío con más de un pedido (misma OT, mismo destinatario —
#envios/services.py::validar_pedidos_para_despacho ya exige mismo RUT para agrupar). Usa el primer
#pedido como referencia para los datos compartidos (contacto, dirección, razón social).
def _template_despacho_grupal(pedidos, folio, track_url):
    anio = datetime.datetime.now().year
    display_folio = folio if folio else "Sin definir"
    referencia = pedidos[0]

    razon_social_line = ""
    if referencia.razon_social:
        razon_social_line = f'<p style="margin: 0 0 10px 0; font-size: 16px;"><strong>Razón Social:</strong> {referencia.razon_social}</p>'

    direccion = _generar_direccion(referencia)
    direccion_line = ""
    if direccion:
        direccion_line = f'<p style="margin: 0 0 10px 0; font-size: 16px;"><strong>Dirección de entrega:</strong> {direccion}</p>'

    pedidos_html = "".join(
        f'<li style="font-size: 16px; margin-bottom: 6px;">Pedido <strong>#{pedido.num_pedido}</strong></li>'
    ...
    <div style="background-color: #f4f7f6; border-radius: 8px; padding: 20px; margin-bottom: 25px; border-left: 4px solid #95c11f;">
      ...
      {direccion_line}
      <p style="margin: 0 0 10px 0; font-size: 16px;"><strong>Estado:</strong> Despachado</p>
      <p style="margin: 0 0 10px 0; font-size: 16px;"><strong>Transportista:</strong> {referencia.get_courier_display()}</p>
      <p style="margin: 0 0 0 0; font-size: 16px;"><strong>Orden de transporte:</strong> {display_folio}</p>
    </div>
    """
...
#Un solo correo para un envío con más de un pedido (misma OT), en vez de uno repetido por pedido.
#Solo aplica a despacho a courier — retiro en tienda nunca agrupa (cada pedido va solo).
def enviar_notificacion_despacho_grupal(pedidos):
    if not settings.EMAIL_SENDER or not settings.EMAIL_PASSWORD:
        raise ValueError("[!] Error: faltan credenciales SMTP (EMAIL_SENDER/EMAIL_PASSWORD en .env).")

    referencia = pedidos[0]
    if referencia.envio_id is None:
        raise ValueError(f"[!] Error: Pedido {referencia.origen}-{referencia.num_pedido} es Despacho pero no tiene un envío asignado todavía.")

    correos_contacto = list(dict.fromkeys(p.email_contacto for p in pedidos if p.email_contacto))
    if not correos_contacto:
        raise ValueError(f"[!] Error: Ninguno de los pedidos del envío #{referencia.envio_id} tiene email de contacto.")

    folio = referencia.envio.orden_transporte
    track_url = _generar_track_url(referencia.get_courier_display(), folio)
    html = _template_despacho_grupal(pedidos, folio, track_url)

    msg = MIMEMultipart("alternative")
    msg["From"] = f"Bioquímica.cl <{settings.EMAIL_SENDER}>"
    msg["To"] = ", ".join(correos_contacto)
    numeros = ", ".join(f"#{pedido.num_pedido}" for pedido in pedidos)
    msg["Subject"] = f"¡Tus pedidos {numeros} van en camino! 🚚"
    msg.attach(MIMEText(html, "html"))

    copia_oculta = [e.strip() for e in settings.EMAIL_BCC.split(",") if "@" in e]
    for pedido in pedidos:
        if pedido.ejecutivo and pedido.ejecutivo.email and pedido.ejecutivo.email not in correos_contacto and pedido.ejecutivo.email not in copia_oculta:
            copia_oculta.append(pedido.ejecutivo.email)

    destinatarios = correos_contacto + copia_oculta
